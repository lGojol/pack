...
class SpecialInventoryWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		## PickItemDialog
		dlgPickETC = uiPickETC.PickEtcDialog()  # Anadido
		...
		self.dlgPickETC = dlgPickETC  # Anadido
	...
	def OnPickItem(self, count):
		itemSlotIndex = self.dlgPickETC.itemGlobalSlotIndex  # Modificado
	...
	def SelectItemSlot(self, itemSlotIndex):
		...
		if mouseModule.mouseController.isAttached():
			...
			if app.SELL == curCursorNum:
				...
				if itemCount > 1:
					self.dlgPickETC.SetTitleName(localeInfo.PICK_ITEM_TITLE)  # Modificado
					self.dlgPickETC.SetAcceptEvent(ui.__mem_func__(self.OnPickItem))  # Modificado
					self.dlgPickETC.Open(itemCount)  # Modificado
					self.dlgPickETC.itemGlobalSlotIndex = itemSlotIndex  # Modificado
	...
	if app.ENABLE_CUBE_RENEWAL:
		def __AddHighlightSlotCube(self, slotIndex):
			if not slotIndex in self.listHighlightedCubeSlot:
				self.listHighlightedCubeSlot.append((slotIndex[0], slotIndex[1], slotIndex[2]))
