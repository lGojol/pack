...
class MoneyInputDialog(ui.ScriptWindow):
	...
	def __CreateDialog(self):
		...
		if app.ENABLE_CHEQUE_SYSTEM:
			...
			self.inputChequeValue.SetNumberMode()  # Asegurar que solo se permitan números
	...
	if app.ENABLE_CHEQUE_SYSTEM:
		...
		def __OnValueUpdate(self):
			...
			if cheque_text:
				...
				if len(cheque_text) > 3:
					cheque_text = cheque_text[:3]
					self.inputChequeValue.SetText(cheque_text)

				if cheque_text.isdigit():
					try:
						cheque = int(cheque_text)
						if cheque > 999:  # Limitar cheques a 999
							cheque = 999
							self.inputChequeValue.SetText(str(cheque))
						elif cheque >= player.CHEQUE_MAX:
							cheque = player.CHEQUE_MAX - 1
							self.inputChequeValue.SetText(str(cheque))
					except ValueError:
						cheque = 0
