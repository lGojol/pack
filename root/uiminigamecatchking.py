...
class CatchKingSimpleWaitingPage(ui.ScriptWindow):
	...
	def __OverInItemSlot(self):
		if self.tooltip_item:
			self.tooltip_item.ClearToolTip()
			self.tooltip_item.AddItemData(CATCHKING_START_ITEM_VNUM, [0, 0, 0], [(0, 0)] * 7)
			self.tooltip_item.ShowToolTip()

	def __OverOutItemSlot(self):
		if self.tooltip_item:
	...
	def __LoadWindow(self):
		...
		try:
			...
			self.itemSlot.SetOverOutItemEvent(ui.__mem_func__(self.__OverOutItemSlot))
