...
class LoginWindow(ui.ScriptWindow):
	...
	def __LoadScript(self, fileName):
		...
		try:
			...
			if app.__BL_MULTI_LANGUAGE_PREMIUM__:
				self.language_board = ui.Window()  # Usamos ui.Window en lugar de ui.ThinBoard
				self.language_board.SetParent(self)
				self.language_board.SetSize(wndMgr.GetScreenWidth(), 70)  # Ajusta el ancho y alto según tus necesidades
				self.language_board.SetPosition(0, 505)  # Ajusta la posición global del contenedor
				self.language_board.Show()  # Mostramos el contenedor, que será transparente
				...
				for i, lang in enumerate(self.language_list):
					...
					btn = ui.Button()
					btn.SetParent(self)  # Los botones son hijos de la ventana principal, no de language_board
					btn.SetPosition(x + 48, 925)  # Coordenadas absolutas en la pantalla
	...
	def __OnClickSelectServerButton(self):
		...
		if app.BL_MOVE_CHANNEL:
			net.ClearServerInfo()
			channelDict = serverInfo.REGION_DICT[regionID][serverID]["channel"]
			channelKeys = sorted([k for k in channelDict.keys() if k >= 0])  # Obtener claves 0, 1, 2, 3
			for i in xrange(1, len(channelKeys) + 1):  # Iterar para IDs 1, 2, 3, 4
				channelKey = channelKeys[i-1]  # Mapear ID 1->clave 0, 2->clave 1, 3->clave 2, 4->clave 3
				_channelName = channelDict[channelKey]["name"]
				net.SetChannelName(i, _channelName.strip())
			
			net.SetChannelName(99, localeInfo.MOVE_CHANNEL_99)
			net.SetServerName(serverName.strip())
