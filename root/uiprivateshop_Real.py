...
class PrivateShopTitleBoard(ui.ShopNameThinboard):
	def __init__(self, type = 0):
		ui.ShopNameThinboard.__init__(self, type, "UI_BOTTOM")
		self.vid = None
		self.type = type
	...
	def __del__(self):
		ui.ShopNameThinboard.__del__(self)
	...
	def Open(self, vid, text):
		...
		self.SetBoardSize(width, height)
		self.Show()
...
class PrivateShopPanel(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			self.priceInputDialog = uiCommon.MoneyInputDialog()
			self.priceInputDialog.SetTitle(localeInfo.PRIVATE_SHOP_INPUT_PRICE_DIALOG_TITLE)
	...
	def RefreshWindow(self):
		...
		if privateShop.IsMainPlayerPrivateShop():
			...
			# Reset the mode as well in case it was something else, ...
			self.mode = self.MODE_DEFAULT
		...
		if privateShop.IsMainPlayerPrivateShop():
			...
			elif (self.mode == self.MODE_BUILD) and page_count > 0:
				self.mode = self.MODE_DEFAULT
			...
			if self.mode == self.MODE_BUILD:
				...
				# Sayfa Sayısını Ayarlama Normal Paket Tek Sayfa
				if self.GetBuildMode() == self.BUILD_DEFAULT:
					self.tab_button_group.Hide()
					page_count = 1

				# Sayfa Sayısını Ayarlama Kaşmir Paket Çift Sayfa
	...
	def OpenBuyItemDialog(self, slotPos):
		...
		# Formatear correctamente el texto
		self.questionDialog.SetText(
			localeInfo.DO_YOU_BUY_ITEM_NEW.format(
				itemName, itemCount, localeInfo.NumberToMoneyString(goldPrice), chequePrice
			)
		)
	...
	def __OpenTitleChangeDialog(self):
		...
		if self.mode == self.MODE_BUILD and not self.__CanBuildPrivateShop():
			chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.PREMIUM_PRIVATE_SHOP_BUILD_RESTRICTED)
			return
...
#Darklovers_Fix
	...
	def OnSelectEmptySlot(self, selectedSlotPos):
		...
		if attachedSlotType == player.SLOT_TYPE_PRIVATE_SHOP:
			...
			if chequePrice:
				self.priceInputDialog.SetChequeValue(chequePrice)
	...
	def AcceptInputPrice(self):
		...
		goldPrice = 0
		if self.priceInputDialog.GetText() != '':
			goldPrice = int(self.priceInputDialog.GetText())

		chequePrice = 0
		if self.priceInputDialog.GetCheque() != '':
			chequePrice = int(self.priceInputDialog.GetCheque())

		if goldPrice <= 0 and chequePrice <= 0:
			return True
