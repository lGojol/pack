...
class GameWindow(ui.ScriptWindow):
	...
	def __BuildKeyDict(self):
		...
		onPressKeyDict[app.DIK_F5]		= lambda : self.interface.wndBio.OpenWindow()
		onPressKeyDict[app.DIK_F6]		= lambda : self.interface.ToggleSwitchbotWindow()
		onPressKeyDict[app.DIK_F7]		= lambda : self.interface.ToggleSpecialInventoryWindow()
		onPressKeyDict[app.DIK_F8]		= lambda : self.interface.TogglePrivateShopPanelWindow()
		onPressKeyDict[app.DIK_F9]		= lambda : self.ToggleWikiWindow()
		onPressKeyDict[app.DIK_F12]		= lambda : self.interface.BuffNPCOpenWindow()
		onPressKeyDict[app.DIK_U]		= lambda : self.interface.OpenFlowerEvent()
		onPressKeyDict[app.DIK_LALT]		= lambda : self.ShowName()
		onPressKeyDict[app.DIK_LCONTROL]	= lambda : self.ShowMouseImage()
		onPressKeyDict[app.DIK_SYSRQ]		= lambda : self.SaveScreen()
		onPressKeyDict[app.DIK_SPACE]		= lambda : self.StartAttack()
		onPressKeyDict[app.DIK_P]	= lambda : self.interface.TogglePreviewCostume()
	...
	def BINARY_NEW_AddAffect(self, type, pointIdx, value, duration):
		# Gestionar affectShower de forma segura
		if hasattr(self, 'affectShower') and self.affectShower:
			self.affectShower.BINARY_NEW_AddAffect(type, pointIdx, value, duration)
	
		# Dragon Soul Deck 1 y 2
		if type in (chr.NEW_AFFECT_DRAGON_SOUL_DECK1, chr.NEW_AFFECT_DRAGON_SOUL_DECK2):
			if hasattr(self, 'interface') and self.interface:
				deck_index = type - chr.NEW_AFFECT_DRAGON_SOUL_DECK1
				self.interface.DragonSoulActivate(deck_index)
	
		# Brave Cape
		if hasattr(app, '__RENEWAL_BRAVE_CAPE__') and app.__RENEWAL_BRAVE_CAPE__:
			if type == chr.NEW_AFFECT_BRAVE_CAPE:
				if (hasattr(self, 'interface') and self.interface and
					hasattr(self.interface, 'wndInventory') and self.interface.wndInventory and
					hasattr(self.interface.wndInventory, 'wndBraveCape') and self.interface.wndInventory.wndBraveCape):
					self.interface.wndInventory.wndBraveCape.LoadData()
	
		# Dragon Soul Qualified
		if type == chr.NEW_AFFECT_DRAGON_SOUL_QUALIFIED:
			if hasattr(self, 'BINARY_DragonSoulGiveQuilification'):
				self.BINARY_DragonSoulGiveQuilification()
	
		# Dragon Soul Set Grade
		if hasattr(app, 'ENABLE_DS_SET') and app.ENABLE_DS_SET and type == chr.NEW_AFFECT_DS_SET:
			if hasattr(self, 'interface') and self.interface:
	...
	def RegisterSpecialEmotion(self, emotionIdx, leftTime, notice):
		...
		if notice != 0:
			chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.OPEN_THE_SPECIAL_ACTION_KIT % constInfo.GET_SPECIAL_EMOTION_NAME(emotionIdx))
		...
		if mouseModule.mouseController.isAttached():
			...
			## Inventory
				...
				if player.ITEM_MONEY == attachedItemIndex:
					#Note: After setting the inventory money, the function that comes in when another user clicks
					if app.ENABLE_CHEQUE_SYSTEM:
	...
	if app.ENABLE_CHEQUE_SYSTEM:
		def __PutMoney(self, attachedType, attachedMoney, dstChrID, cheque):
			if True == chr.HasInstance(dstChrID) and player.GetMainCharacterIndex() != dstChrID:
				net.SendExchangeStartPacket(dstChrID)
				net.SendExchangeElkAddPacket(attachedMoney, cheque)
			else:
				self.stream.popupWindow.Close()
	...
	def __DropItem(self, attachedType, attachedItemIndex, attachedItemSlotPos, attachedItemCount):
		...
		if player.SLOT_TYPE_INVENTORY == attachedType and player.IsEquipmentSlot(attachedItemSlotPos):
			...
			if player.SLOT_TYPE_INVENTORY == attachedType:
				...
				## Dialog
				if app.ENABLE_DELETE_ITEM_WHEN_DROP:
				...
				itemDropQuestionDialog.SetAcceptEvent(lambda arg=True: self.RequestDropItem(arg))
				if app.ENABLE_DELETE_ITEM_WHEN_DROP:
					itemDropQuestionDialog.SetDestroyEvent(ui.__mem_func__(self.RequestDestroyItemCheck))
				itemDropQuestionDialog.SetCancelEvent(lambda arg=False: self.RequestDropItem(arg))
				itemDropQuestionDialog.Open()
				itemDropQuestionDialog.dropType = attachedType
	...
	if app.ENABLE_DELETE_ITEM_WHEN_DROP:
		def OnCloseDropAndQuestionDialog(self):
		...
		def RequestDestroyItemCheck(self):
			self.questionDialog = uiCommon.QuestionDialog()
			self.questionDialog.SetText(localeInfo.DESTROY_ITEM_CHECK)
			self.questionDialog.SetAcceptEvent(lambda arg=True: (self.RequestDestroyItem(arg)))
			self.questionDialog.SetCancelEvent(lambda arg=False: (self.RequestDestroyItem(arg)))
			...
			if answer == True:
				...
				if player.SLOT_TYPE_INVENTORY == dropType:
					if dropNumber == player.ITEM_MONEY:
						...
						self.OnCloseDropAndQuestionDialog()
			else:
				self.OnCloseDropAndQuestionDialog()
	...
	def __ServerCommand_Build(self):
		...
		# bpass
		serverCommandList["BP_CLEAR"] = self.BattlePassClear
		serverCommandList["BP_ACTIVATED"] = self.BattlePassActive
		serverCommandList["BP_UPDATE"] = self.BattlePassUpdate
		serverCommandList["BP_INFO"] = self.BattlePassInfo
		serverCommandList["BP_INFOR"] = self.BattlePassInfoReward
		serverCommandList["BP_INFOG"] = self.BattlePassInfoGeneral
		serverCommandList["BP_SHOP_COIN"] = self.ShowShopCoinBattlePass
		...
		if app.ENABLE_BATTLE_ROYALE:
			serverCommandList["BattleRoyaleJoinInfo"]	= self.__BattleRoyaleJoinInfo
			serverCommandList["BattleRoyaleWarpInfo"]	= self.__BattleRoyaleWarpInfo
			serverCommandList["BattleRoyaleZoneInfo"]	= self.__BattleRoyaleZoneInfo
			serverCommandList["BattleRoyaleKillInfo"]	= self.__BattleRoyaleKillInfo
			serverCommandList["BattleRoyalePlayersInfo"]	= self.__BattleRoyalePlayersInfo
	...
	if app.ELEMENT_SPELL_WORLDARD:
		...
		def ElementsSpellChangeSuceeded(self,vnum,element_type):
			...
			ELEMENT_NAME = {
				item.APPLY_ENCHANT_ELEC : uiScriptLocale.REFINE_ELEMENT_CHANGE_ELECT,
	...
	if app.ENABLE_CUBE_RENEWAL:
		def BINARY_CUBE_RENEWAL_OPEN(self, vnum):
			if self.interface:
				self.interface.BINARY_CUBE_RENEWAL_OPEN(vnum)
	...
	def __bioUpdate(self, v1,v2,v3,v4):	
		...
		if int(v1) == 0: ##bio level equals 0
	...
	if app.ENABLE_MINI_GAME_CATCH_KING:
		def MiniGameCatchKingEvent(self, enable):
			player.SetCatchKingGame(int(enable))
	...
	if app.ENABLE_MINI_GAME_RUMI:
		...
		def __MiniGameOkey(self, enable):
			player.SetRumiGame(int(enable))

			if self.interface:
				self.interface.IntegrationEventBanner()

		def __MiniGameOkeyNormal(self, enable):
			player.SetMiniGameOkeyNormal(int(enable))
	...
	## DayMode
		def __PRESERVE_DayMode_Update(self, mode):
			import dbg
			dbg.TraceError("Entering __PRESERVE_DayMode_Update with mode: %s" % mode)
			if app.ENABLE_ENVIRONMENT_EFFECT_OPTION:
				dbg.TraceError("Checking IsBoomMap: %s" % background.IsBoomMap())
				if not background.IsBoomMap():
					dbg.TraceError("IsBoomMap returned False, exiting __PRESERVE_DayMode_Update")
					return
			else:
				dbg.TraceError("Checking __IsXMasMap: %s" % self.__IsXMasMap())
				if not self.__IsXMasMap():
					dbg.TraceError("__IsXMasMap returned False, exiting __PRESERVE_DayMode_Update")
					return
		
			try:
				if "light" == mode:
					background.SetEnvironmentData(background.DAY_MODE_LIGHT)
					dbg.TraceError("background.SetEnvironmentData(LIGHT) executed successfully")
				elif "dark" == mode:
					...
					dbg.TraceError("background.SetEnvironmentData(DARK) executed successfully")
			except Exception as e:
				dbg.TraceError("Error in background.SetEnvironmentData: %s" % str(e))

		def __DayMode_Update(self, mode):
			import dbg
			dbg.TraceError("Entering __DayMode_Update with mode: %s" % mode)
			if not hasattr(self, 'curtain'):
				dbg.TraceError("Error: self.curtain is not initialized")
				return
		
			try:
				if "light" == mode:
					dbg.TraceError("Calling curtain.SAFE_FadeOut for Light mode")
					self.curtain.SAFE_FadeOut(self.__DayMode_OnCompleteChangeToLight)
				elif "dark" == mode:
					dbg.TraceError("Calling curtain.SAFE_FadeOut for Dark mode")
					self.curtain.SAFE_FadeOut(self.__DayMode_OnCompleteChangeToDark)
			except Exception as e:
				dbg.TraceError("Error in curtain.SAFE_FadeOut: %s" % str(e))

	def __DayMode_OnCompleteChangeToLight(self):
		background.SetEnvironmentData(0)
		self.curtain.FadeIn()

	def BattlePassActive(self, isPremium):
		if int(isPremium) == 1:
			self.interface.wndBattlePass.UnlockPremium()

	def BattlePassUpdate(self, index, count, exp, totalExp, level, coolTime):
		self.interface.wndBattlePass.SetExperience(int(exp), int(totalExp))
		self.interface.wndBattlePass.SetLevel(int(level))
		self.interface.wndBattlePass.UpdateMission(int(index), int(count), int(coolTime))
	
	def BattlePassInfo(self, category, index, title, type, vnum, count, maxCount, expRecv, isPremium, coolTime):
		self.interface.wndBattlePass.AppendMission(int(category), int(index), title, int(type), int(vnum), int(count), int(maxCount), int(expRecv), int(isPremium), int(coolTime))
		
	def BattlePassInfoReward(self, index, vnum, count, iNecessaryLevel, status, isPremium):
		self.interface.wndBattlePass.AppendRewardItem(int(index), int(vnum), int(count), int(status), int(isPremium), int(iNecessaryLevel))

	def BattlePassInfoGeneral(self, isPremium, monthBattlePass, EXP, totalEXP, level, coins):
		if int(isPremium) == 1:
			self.interface.wndBattlePass.UnlockPremium()
		...
		# constInfo.BATTLE_PASS_COINS = int(coins)

	def ShowShopCoinBattlePass(self):
		self.interface.dlgShop.wndTextBattlePassCoin.Show()
		self.interface.dlgShop.btnBuy.Hide()
		self.interface.dlgShop.btnSell.Hide()
	
	def BattlePassClear(self):
		self.interface.wndBattlePass.ClearBattlePass()
	...
	if app.CRYSTAL_EVENT_SYSTEM:
		def BINARY_CrystalEvent_OnRecvData(self, leftTime, membersOnlineRed, membersOnlineBlue, crystalsRed, crystalsBlue):
			self.interface.AddCrystalResultWindow(leftTime, membersOnlineRed, membersOnlineBlue, crystalsRed, crystalsBlue)
	...
	if app.ENABLE_PREMIUM_PRIVATE_SHOP:
		...
		def PrivateShopRefreshResult(self):
			if self.interface:
				self.interface.PrivateShopRefreshResult()
	...
	if app.ENABLE_BATTLE_ROYALE:
		def __BattleRoyaleJoinInfo(self, iSec):
			if self.interface:
				constInfo.BATTLE_ROYALE_CAN_WARP = False
				iSec = int(iSec)
				if iSec == -1:
					self.interface.HideBattleRoyaleInfo()
				else:
					self.interface.OpenBattleRoyalInfo(iSec)
					
		def __BattleRoyaleWarpInfo(self, iSec, playersCount):
			if self.interface:
				iSec = int(iSec)
				playersCount = int(playersCount)
				constInfo.BATTLE_ROYALE_CAN_WARP = True
				self.interface.BattleRoyaleWarpInfo(iSec, playersCount)
				
		def __BattleRoyaleZoneInfo(self, iSec, isFirst):
			if self.interface:
				iSec = int(iSec)
				isFirst = int(isFirst)
				self.interface.BattleRoyaleZoneInfo(iSec, isFirst)
				
		def __BattleRoyaleKillInfo(self, killCount):
			if self.interface:
				killCount = int(killCount)
				self.interface.BattleRoyaleKillInfo(killCount)
				
		def __BattleRoyalePlayersInfo(self, remainingPlayers):
			if self.interface:
				remainingPlayers = int(remainingPlayers)
				self.interface.BattleRoyalePlayersInfo(remainingPlayers)
					
		def BIN_BattleRoyaleRefreshTerrain(self):
			if constInfo.BATTLE_ROYALE_TERRAIN_ZONE:
	...
	if app.__BL_LUCKY_BOX__:
		def ShowLuckyBoxWindow(self, dwItemVnum, byItemCount, iNeedMoney):
			if self.interface:
				self.interface.ShowLuckyBoxWindow(dwItemVnum, byItemCount, iNeedMoney)
