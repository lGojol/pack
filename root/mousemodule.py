...
class CMouseController(object):
	...
	def __init__(self):
		...
		self.callbackDict = {}
		self.CreateNumberLine()  # Inicializar en el constructor
				...
				if Type == player.SLOT_TYPE_AUTO:
					if SlotNumber <= player.AUTO_SKILL_SLOT_MAX:
						...
						if app.ENABLE_EXTEND_INVEN_SYSTEM:
							itemIndex = player.GetItemIndex(player.SLOT_TYPE_INVENTORY, ItemIndex)
						else:
		...
		if True == self.isAttached():
			if 0 != self.AttachedIconHandle:
				...
				if self.countNumberLine:
					...
					dbg.TraceError("countNumberLine is None in Update")

				if app.ENABLE_CHEQUE_SYSTEM:
					if self.chequeNumberLine and self.chequeNumberLine.IsShow():
						self.chequeNumberLine.SetPosition(self.x, self.y - self.AttachedIconHalfHeight - 13)
					else:
						if not self.chequeNumberLine:
							dbg.TraceError("chequeNumberLine is None in Update")
