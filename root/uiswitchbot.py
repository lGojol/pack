...
class SwitchbotItemTab(ui.ThinBoardCircle):
	...
	def OverInItem(self, overSlotPos):
		...
		if app.ENABLE_APPLY_RANDOM:
			apply_random_list = [player.GetItemApplyRandom(player.SWITCHBOT, self.slot_num, i) for i in xrange(player.APPLY_RANDOM_SLOT_MAX_NUM)]

		if app.ELEMENT_SPELL_WORLDARD:
			self.tooltipItem.FuncElementSpellItemDate(self.slot_num, player.SWITCHBOT)

		#self.tooltipItem.AddItemData(itemVnum, metinSlot, attrSlot, 0, 0, player.SWITCHBOT, self.slot_num, apply_random_list)
		self.tooltipItem.AddItemData(itemVnum, metinSlot, attrSlot, None, 0, 0, player.SWITCHBOT, self.slot_num, None, apply_random_list, 0)
	...
	def RefreshItemSlot(self):	
		if self.IsEmpty():
			...
			self.statusBar.Show()
			if self.IsActive():
				# self.itemSlot.ActivateSlot(0)  # Comentamos o eliminamos esta línea para desactivar el efecto visual
				self.statusBar.SetColor(COLOR_INACTIVE)  # Cambiamos COLOR_ACTIVE por COLOR_INACTIVE para evitar el cambio de color
			else:
