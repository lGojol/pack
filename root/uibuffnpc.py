...
class BuffNPCWindow(ui.ScriptWindow):
	...
	def ClickChangeSexButton(self):
		if self.isActive == True:
			if player.GetItemIndex(player.BUFF_EQUIPMENT, 10) == 0:
	...
	def ClickResetSkillButton(self):
		if player.GetItemIndex(player.BUFF_EQUIPMENT, 11) == 0:
	...
	def SetPreviewUpgradeSkillSlot(self, itemVnum):
		if 50300 == itemVnum:
			...
			self.upgradeBookSlotVnum = player.GetItemMetinSocket(player.BUFF_EQUIPMENT, 6, 0)
		elif 50513 == itemVnum:
...
class BuffNPCCreateWindow(ui.ScriptWindow):
	...
	def __OnClickChangeSexButton(self, sex):
		...
		if sex == 0:
			...
			renderTarget.SelectModelBuff(TARGET_RENDER_INDEX_FIRST_USE, 7, 7, 40196, 41912, 45001, 85004, 49016)  # Masculino: race=7, vnum=7, hair=45001
		if sex == 1:
			self.selectSexButton[0].SetUp()
			self.selectSexButton[1].Down()
			renderTarget.SelectModelBuff(TARGET_RENDER_INDEX_FIRST_USE, 3, 3, 40196, 41914, 45002, 85004, 49016)  # Femenino: race=3, vnum=3, hair=45002
...
class BuffNPCReNameWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		try:
			...
			self.editline_name = self.GetChild("editline_name")
			
			# Verificar que todos los elementos se inicializaron
			if self.editline_name is None:
				raise Exception("editline_name no inicializado tras GetChild")
			if self.board is None or self.accept_button is None or self.cancel_button is None:
				raise Exception("Uno o mas elementos de la interfaz no se inicializaron")
			
			# Configurar eventos
			self.board.SetCloseEvent(self.Close)
			self.accept_button.SetEvent(self.ClickAcceptButton)
			self.cancel_button.SetEvent(self.Close)
			
		except KeyError as e:
			import exception
			exception.Abort("BuffNPCReNameWindow.__LoadWindow: No se pudo cargar el elemento %s" % str(e))
		except Exception as e:
			import exception
			exception.Abort("BuffNPCReNameWindow.__LoadWindow: Error general: %s" % str(e))

	def Show(self):
		# Re-cargar la ventana si los elementos no están inicializados
		if self.editline_name is None or self.board is None or self.accept_button is None or self.cancel_button is None:
			self.__LoadWindow()
			if self.editline_name is None or self.board is None or self.accept_button is None or self.cancel_button is None:
				chat.AppendChat(chat.CHAT_TYPE_INFO, "Error: Elementos de la interfaz no inicializados.")
				self.Close()
				return
		self.editline_name.SetText("")
		self.editline_name.SetFocus()
		ui.ScriptWindow.Show(self)
	...
	def ClickAcceptButton(self):
		if self.editline_name is None:
			chat.AppendChat(chat.CHAT_TYPE_INFO, "Error: 'editline_name' no inicializado.")
			self.Close()
			return
		if not self.editline_name.GetText():
			chat.AppendChat(chat.CHAT_TYPE_INFO, uiScriptLocale.ASLAN_BUFF_CHAT_INFO_NAME_IS_EMPTY)
			return
		net.SendBuffNPCCreate(2, self.editline_name.GetText(), 0)
		self.Close()
