...
class RumiWaitingPage(ui.ScriptWindow):
	...
	def __init__(self):
		...
		# Almacenar los vnums para cada slot
		self.rumi_card_piece_slot_vnum = item.ITEM_VNUM_RUMI_CARD_PIECE
		self.rumi_card_slot_vnum = item.ITEM_VNUM_RUMI_CARD_PACK
		self.__LoadWindow()

	def __del__(self):
		ui.ScriptWindow.__del__(self)

	def __SlotOverInItemPiece(self, slot):
		if self.tooltip:
			self.tooltip.SetItemToolTip(self.rumi_card_piece_slot_vnum)

	def __SlotOverInItemPack(self, slot):
		if self.tooltip:
			self.tooltip.SetItemToolTip(self.rumi_card_slot_vnum)
	...
	def __LoadWindow(self):
		...
		try:
			...
			# Usar funcion especifica para rumi_card_piece_slot
			self.rumi_card_piece_slot.SetOverInItemEvent(ui.__mem_func__(self.__SlotOverInItemPiece))
			...
			# Usar funcion especifica para rumi_card_slot
			self.rumi_card_slot.SetOverInItemEvent(ui.__mem_func__(self.__SlotOverInItemPack))
			...
			if localeInfo.IsARABIC():
				...
				self.prev_button.SetWindowHorizontalAlignLeft()
				prev_button_width = self.prev_button.GetWidth()
				(prev_button_lx, prev_button_ly) = self.prev_button.GetLocalPosition()
				self.prev_button.SetPosition(prev_button_lx - prev_button_width, prev_button_ly)

				self.next_button.SetWindowHorizontalAlignLeft()
				next_button_width = self.next_button.GetWidth()
				(next_button_lx, next_button_ly) = self.next_button.GetLocalPosition()
				self.next_button.SetPosition(next_button_lx - next_button_width, next_button_ly)
		except:
			import exception
	...
	def __ClickStartButton(self):
		if player.GetItemCountByVnum(item.ITEM_VNUM_RUMI_CARD_PACK) <= 0 or player.GetElk() < RUMI_START_GOLD:
	...
	def Show(self):
		...
		event.SetRestrictedCount(self.desc_index, 50)
		event.AllProcesseEventSet(self.desc_index)
		...
		self.DecreaseMiniGameRumiCardCount()
	...
	def DecreaseMiniGameRumiCardCount(self):
		self.rumi_card_piece_count_text.SetText("%d/%d" % (player.GetItemCountByVnum(item.ITEM_VNUM_RUMI_CARD_PIECE), RUMI_CARD_PIECE_COUNT_MAX))
		self.rumi_card_count_text.SetText("%d/%d" % (player.GetItemCountByVnum(item.ITEM_VNUM_RUMI_CARD_PACK), RUMI_CARD_COUNT_MAX))
