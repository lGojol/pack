...
class Window(ui.ScriptWindow):
	...
	def LoadWindow(self):
		...
		for i in xrange(6):
			...
			line = ui.MakeImageBox(self, "d:/ymir work/ui/game/guild/dragonlairranking/line_down.sub", 22, yPos)
			...
			TIME_BACKGROUND.SetPosition(237, 2)
		...
		#self.AddRank(1, "Staff", "24", "23 minutes")
		self.AddRank(5, "-", "-", "-", "-")
		...
		if self.rankData != None:
			if self.rankData["ID"][line]:
				...
				if player.GetGuildName() == str(name):
					self.rankLine[line].LoadImage("d:/ymir work/ui/game/guild/dragonlairranking/line_up.sub")
				else:
					self.rankLine[line].LoadImage("d:/ymir work/ui/game/guild/dragonlairranking/line_down.sub")
