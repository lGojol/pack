	...
	def AskBuyItem(self, slotPos):
		...
		if app.ENABLE_CHEQUE_SYSTEM:
			...
			if shop.GetBuyWithItem(slotPos) != 0:
				itemBuyQuestionDialog.SetText(localeInfo.DO_YOU_BUY_ITEM(itemName, itemCount, localeInfo.NumberToWithItemString(shop.GetBuyWithItemCount(slotPos), shop.GetItemCheque(slotPos), item.GetItemName())))
			else:
		...
		elif app.ENABLE_CHEQUE_SYSTEM and not (app.ENABLE_BATTLE_FIELD or app.ENABLE_MEDAL_OF_HONOR):
			itemBuyQuestionDialog.SetText(localeInfo.DO_YOU_BUY_ITEM(itemName, itemCount, localeInfo.NumberToMoneyString(itemPrice), shop.GetItemCheque(slotPos)))
		else:
