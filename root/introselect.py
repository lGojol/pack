...
class MyCharacters:
	...
	def MakeCharacter(self, slot, name, race, form, hair, acce, aura):
		...
		# Reiniciar aura para esta instancia
		if app.ENABLE_AURA_SYSTEM:
			chr.SetAura(0)  # Desactivar aura por defecto
		...
		if app.ENABLE_ACCE_SYSTEM:
			chr.SetAcce(acce)
		if app.ENABLE_AURA_SYSTEM:
			chr.SetAura(aura)  # Aplicar el aura específico del personaje
...
#	else:
	...
	def SelectButton(self, slot):
		...
		# Limpiar todas las instancias de personajes antes de seleccionar una nueva
		for i in xrange(self.mycharacters.GetMyCharacterCount()):
			chr.SelectInstance(i)
			if app.ENABLE_AURA_SYSTEM:
				chr.SetAura(0)  # Desactivar aura para todas las instancias
			chr.Hide()
			#if app.ENABLE_RENDER_LOGIN_EFFECTS:
			#	chr.ShowEffects(False)
	
		# Seleccionar y configurar la instancia del personaje actual
		chr.SelectInstance(self.SelectSlot)
		if app.ENABLE_AURA_SYSTEM:
			aura = self.mycharacters.myUnitDic[self.RealSlot[self.SelectSlot]].GetUnitData()["AURA"]
			chr.SetAura(aura)  # Aplicar el aura específico del personaje seleccionado
		chr.Show()
		#if app.ENABLE_RENDER_LOGIN_EFFECTS:
		#	chr.ShowEffects(True)
		...
		# Configuración de nombres y alfa
		...
		# Configuración de imágenes de cara y colores de fuente
