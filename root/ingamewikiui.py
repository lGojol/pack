...
class WikiMainCostumeWindow(ui.Window):
	...
	def Show(self, vnums):
		...
		extractedLists = []
		for i in vnums:
			if isinstance(i, (list, tuple)):  # Solo verificar list y tuple
				extractedLists.append(list(i))  # Convertir a lista si es tuple
		...
		# Convertir todos los elementos de vnums a enteros
		vnums = [int(x) for x in vnums if isinstance(x, (int, str)) and str(x).isdigit()]
		
		isChanged = (len(vnums) != len(self.costumeVnums))
	...
	def OnOverIn(self, vnum, metinSlot=[0 for i in xrange(player.METIN_SOCKET_MAX_NUM)]):
		...
		try:
			vnum = int(vnum)  # Convertir vnum a entero para evitar tipos no hashables
		except (ValueError, TypeError):
			return  # Si vnum no es convertible a entero, salir para evitar errores
		...
		# Limpiar el renderizado previo y asegurarse de que no esté en el tooltip
		self.wikiRenderTarget.Hide()
		if self.wikiRenderTarget in self.toolTip.childrenList:
			self.toolTip.childrenList.remove(self.wikiRenderTarget)
		
		# Solo mostrar renderizado para mascotas y monturas
		if vnum in ingamewikiconfig.COSTUME_PET_VNUMS or subType == item.COSTUME_TYPE_MOUNT:
			...
			if vnum in ingamewikiconfig.COSTUME_PET_VNUMS:
				self.wikiRenderTarget.SetModel(ingamewikiconfig.COSTUME_PET_VNUMS[vnum])  # Usa el VNUM de la mascota
			elif subType == item.COSTUME_TYPE_MOUNT:
		...
		# Siempre mostrar la descripción del elemento
		self.toolTip.AddItemData(vnum, metinSlot, 0)
